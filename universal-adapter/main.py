...
class PackageManager:
    ...
    async def install_package(self, package_spec: PackageSpec) -> bool:
        ...
        logger.info(
            "📥 Installing package: %s - %s", package_spec.type, package_spec.source
        )
...
class UniversalMCPAdapter:
    ...
    def _load_config(self):
        ...
        try:
            ...
            logger.info("📋 Server capabilities: tools=%d, resources=%d, prompts=%d", len(self.capabilities['tools']), len(self.capabilities['resources']), len(self.capabilities['prompts']))
    ...
    async def initialize(self):
        ...
        if package_spec:
            logger.info("📦 Detected package requirement: %s - %s", package_spec.type, package_spec.source)
    ...
    try:
        ...
        # Handle resource methods dynamically based on capabilities
            ...
            if adapter._supports_resources():
                ...
                logger.info("📂 Resource method '%s' - forwarding to stdio server", method)
                try:
                    response = await adapter.send_request(request_data)
                    logger.debug("✅ Resource method '%s' completed successfully", method)
                    return response
                except Exception as e:
                    logger.error("❌ Failed to forward resource method '%s': %s", method, e)
                    return {
                        "jsonrpc": "2.0",
                ...
                logger.info("🚫 Resource method '%s' not supported by server", method)
                return {
                    "jsonrpc": "2.0",
                    "id": request_id or "unknown",
                    "error": {
        ...
        # Handle prompt methods dynamically based on capabilities
            ...
            if adapter._supports_prompts():
                ...
                logger.info("💬 Prompt method '%s' - forwarding to stdio server", method)
                try:
                    response = await adapter.send_request(request_data)
                    logger.debug("✅ Prompt method '%s' completed successfully", method)
                    return response
                except Exception as e:
                    logger.error("❌ Failed to forward prompt method '%s': %s", method, e)
                    return {
                        "jsonrpc": "2.0",
                        "id": request_id or "unknown",
                        "error": {
                ...
                logger.info("🚫 Prompt method '%s' not supported by server", method)
                return {
                    "jsonrpc": "2.0",
                    "id": request_id or "unknown",
                    "error": {
        ...
        # Handle tool methods dynamically based on capabilities
            ...
            if adapter._supports_tools():
                ...
                logger.info("🔧 Tool method '%s' - forwarding to stdio server", method)
                try:
                    response = await adapter.send_request(request_data)
                    logger.debug("✅ Tool method '%s' completed successfully", method)
                    return response
                except Exception as e:
                    logger.error("❌ Failed to forward tool method '%s': %s", method, e)
                    return {
                        "jsonrpc": "2.0",
                        "id": request_id or "unknown",
                        "error": {
                ...
                logger.info("🚫 Tool method '%s' not supported by server", method)
                return {
                    "jsonrpc": "2.0",
                    "id": request_id or "unknown",
                    "error": {
