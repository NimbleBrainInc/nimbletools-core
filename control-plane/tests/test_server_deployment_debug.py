...
class TestServerDeploymentDebug:
    ...
    def test_deploy_echo_to_workspace_exact_scenario(
        ...
        with patch("nimbletools_control_plane.middlewares.client.CoreV1Api") as mock_k8s_core_class:
            ...
            with patch(
                ...
                assert (
                    created_service["spec"]["container"]["image"] == "nimbletools/mcp-echo:latest"
                )
    ...
    async def test_workspace_access_validator_function(self):
        ...
        # TODO: Fix this test - create_workspace_access_validator no longer exists
        validator = None  # Placeholder to fix undefined name error
            ...
            with patch("nimbletools_control_plane.routes.servers.log_operation_start"):
                with patch(
                    ...
                    with patch(
                        ...
                        mock_k8s.return_value.create_namespaced_custom_object.return_value = None
                        ...
                        if response.status_code != 200:
                            pytest.fail(f"Server deployment failed at step 2: {response.json()}")

    async def test_workspace_namespace_resolution(self, client: TestClient, mock_k8s_config):
        ...
        # Create mock request
        mock_request = Mock()
        mock_request.path_params = {"workspace_id": "6587baf7-183a-405f-98c3-612919101dcf"}
        ...
        with patch("nimbletools_control_plane.middlewares.client.CoreV1Api") as mock_k8s_core_class:
            ...
            # Mock namespace response
            mock_namespace = Mock()
            mock_namespace.metadata.name = "ws-foobar-6587baf7-183a-405f-98c3-612919101dcf"
            mock_namespaces = Mock()
            mock_namespaces.items = [mock_namespace]
            mock_k8s_core.list_namespace.return_value = mock_namespaces
            ...
            with patch("nimbletools_control_plane.routes.servers.log_operation_start"):
                with patch(
                    ...
                    with patch(
                        ...
                        if response.status_code != 200:
                            pytest.fail(f"Real workspace deployment failed: {response.json()}")

                        # Verify the correct namespace was used
                        call_args = mock_k8s_custom.create_namespaced_custom_object.call_args[1]
                        assert call_args["namespace"] == f"ws-foobar-{real_workspace_id}"
