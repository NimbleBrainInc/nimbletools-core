...
- Server deployment status now accurately reflects failures like ImagePullBackOff and CrashLoopBackOff
- Control-plane properly detects failed deployments by checking deployment conditions and pod status
- Server list and detail endpoints now distinguish between Pending (starting) and Failed (error) states

### Changed

- Deployment phase determination examines Kubernetes deployment conditions and pod waiting reasons
- Status logic returns "Unknown" on transient API errors instead of falling back to stale MCPService status
- Added comprehensive unit tests for deployment phase detection covering failure scenarios
