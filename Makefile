...
test-cov-control-plane: ## Run control plane tests with coverage
	...
	@cd control-plane && uv run pytest tests/ --cov=nimbletools_control-plane --cov-report=html --cov-report=term-missing
...
type-check-control-plane: ## Type check control plane
	...
	@cd control-plane && uv run mypy --package nimbletools_control-plane
...
github-release: ## Create GitHub release (requires gh CLI)
	...
	@gh release create v$(VERSION) \
		--title "NimbleTools Core v$(VERSION)" \
...
release: check clean-dist check-publish tag publish ## Complete release workflow
	...
	@echo "1. Check GitHub releases: https://github.com/nimblebrain/nimbletools-core/releases"
	@echo "2. Check Docker Hub: https://hub.docker.com/r/nimbletools"
	@echo "3. Check GHCR: https://github.com/nimblebrain/nimbletools-core/pkgs/container/charts%2F$(CHART_NAME)"
	@echo "4. Test installation: curl -sSL https://raw.githubusercontent.com/nimblebrain/nimbletools-core/main/install.sh | bash"